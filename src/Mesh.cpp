...
namespace war {
	...
	bool Mesh::aabbTriangleHit(const aabb_t& mbb, const Triangle& tri) const {
		...
		const point_t center = mbb.min + e / 2.0f;
	...
	const grid_ptr Mesh::getGrid() const { return grid; }
	...
	bool Mesh::Loader::OBJ(const std::string& filename) {
		...
		// init grid
		mesh->grid = std::make_shared<grid_t>(min, max, index_t(4));
	...
	const mesh_ptr Mesh::Loader::getMesh() const { return mesh; }
} // namespace war
