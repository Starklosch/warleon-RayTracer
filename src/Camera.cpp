...
Camera::Camera(const Ray&r,scalar_t d,scalar_t w,scalar_t h,scalar_t t):ray(r),distance(d),width(w),height(h),tilt(t){
		initCameraAxes();
	}

void Camera::initCameraAxes(){
	rotationMatrix_t rotation = glm::rotate(rotationMatrix_t(1.0),glm::radians(tilt),ray.D);
